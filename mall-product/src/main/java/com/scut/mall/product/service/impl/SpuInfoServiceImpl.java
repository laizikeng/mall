...
import com.scut.mall.product.entity.*;
import com.scut.mall.product.feign.CouponFeignService;
import com.scut.mall.product.feign.SearchFeignService;
import com.scut.mall.product.feign.WareFeignService;
import com.scut.mall.product.service.*;
import com.scut.mall.product.vo.*;
...
import com.scut.mall.product.dao.SpuInfoDao;
...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    public void saveSpuInfo(SpuSaveVo vo) {
        ...
        // 保存spu的图片集  pms_sku_images
        // 先获取所有图片
        List<String> images = vo.getImages();
        // 保存图片的时候 并且保存这个是哪个spu的图片
        spuImagesService.saveImages(spuInfoEntity.getId() ,images);

        // 保存spu的规格属性  pms_product_attr_value
        List<BaseAttrs> baseAttrs = vo.getBaseAttrs();
        List<ProductAttrValueEntity> collect = baseAttrs.stream().map(attr -> {
            ProductAttrValueEntity valueEntity = new ProductAttrValueEntity();
            valueEntity.setAttrId(attr.getAttrId());
            // 可能页面没有传入属性名字 根据属性id查到所有属性 给名字赋值
